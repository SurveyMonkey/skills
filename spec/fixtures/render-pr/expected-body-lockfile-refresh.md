...
- Resolves 2 Dependabot alert(s) for `lodash` in the 4.x line by refreshing the lockfile
...
The fix is a no-change lockfile refresh: the manifest already admits the fixed version, but the committed lockfile pinned `4.17.18` on the 4.x line. Re-resolving it against the unchanged manifests moves it to `4.17.21`.
