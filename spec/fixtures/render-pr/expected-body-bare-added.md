...
- Resolves 2 Dependabot alert(s) for `lodash` in the 4.x line by adding an unscoped override
...
Added an unscoped override `lodash: ">=4.17.21 <5"`. Scoped entries were tried on: express, astro. Resolved copies after the fix: 4.17.21.

Scoped entries on `express` and `astro` left one copy of `lodash` resolving under
`next > express`, and no single parent covered both paths, so the pin went unscoped.
