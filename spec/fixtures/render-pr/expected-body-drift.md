...
The first commit is a no-change lockfile refresh — the default branch's lockfile is stale relative to its manifests, so any install re-resolves these entries; the second commit is the fix.
