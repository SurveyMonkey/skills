...
- Fix subagent worktrees live in temp directories outside the target repository, never `.claude/worktrees/` inside it: an in-repo worktree shows as untracked noise in any repo not ignoring the path, and a crashed agent would strand a checkout inside someone's project ([ADR 003](../adr/003-worktree-isolation-and-concurrency-cap.md)).
