...
- ~~Org scope via the aggregate endpoint; user scope via deterministic per-repo fan-out.~~ **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): there is no scope above the checkouts on disk. `discover-repos.sh` answers the current repository, or every repository checked out directly under the current directory, and the skill never clones to widen that set.
...
**Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): ~~`discover-alerts.sh # extended: --scope repo|org|user`~~ above never ships. The script stays repo-only and `scripts/common/` gains `discover-repos.sh`, which answers scope from the checkouts on disk.
...
1. **Detect scope.** ~~`detect-scope.sh` maps the working directory to repo, org, or user scope using the existing `@`-segment convention. The user can override by asking explicitly ("fix alerts across the whole org").~~ **Superseded** ([#134](https://github.com/SurveyMonkey/skills/issues/134)): scope comes from git, not from directory names. `detect-scope.sh` answers `repo` when the working directory is inside a git repository, with `nwo` parsed from `origin`'s remote URL as its only source, and `null` when it is not — a workspace's `@`-owner layout is one machine's convention, and reading it as scope silently gave a plain `~/projects/my-repo` checkout user scope against the wrong target. ~~On a null scope the orchestrator asks the user what to operate on (an org, the user's own repos, or one named repo), and there is no override to make at repo scope: being in a checkout is what repo scope means. Where a new clone lands is asked once per run in step 5's checkout resolution rather than derived from the convention; the convention survives only as a suggested default in that question.~~ **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): scope is now the checkouts on disk, answered by `discover-repos.sh` as the current checkout root or every immediate non-dot subdirectory that is itself a checkout root. There is no question about what to operate on, no clone, and therefore no clone destination to ask about; an empty list simply stops the run. `detect-scope.sh` itself is unchanged; what narrows is its use here, to reading one checkout's `nwo` and `default_branch`, once per checkout that discovery found.
2. **Discover.** ~~`discover-alerts.sh --scope <scope>`~~ **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): `--scope` is gone and `discover-alerts.sh` is repo-only, run once per checkout discovery returned. It fetches, groups, ranks, and PR-checks alerts (see endpoint strategy below). Output is the existing `{actionable, skipped}` contract, with each group additionally carrying `repo` so cross-repo dispatch works.
...
**Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): only the repo row survives. The set of repositories in a run comes from `discover-repos.sh` reading the filesystem, not from an aggregate endpoint, so there is no org or user endpoint strategy, no push-access filtering and no `skipped_repos` output. A checkout the run cannot use, because it has no usable origin, no resolvable default branch, or a failed branch-namespace probe, is still reported by name and excluded; the difference is that the run only ever considers repositories the user has already cloned.
...
| Phase 3 | `discover-alerts.sh` | Already exists; ~~gains `--scope`~~ **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): stays repo-only; scope is answered by `discover-repos.sh` from the checkouts on disk |
...
- ~~**Org endpoint permissions.** `GET /orgs/{org}/dependabot/alerts` requires org-level security visibility (security manager or admin). The script must detect a 403 and fall back to per-repo enumeration of repos the user can access, or report the limitation clearly.~~ **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): the org endpoint and its 403 fallback are gone; discovery reads one checkout's alerts at a time. [#37](https://github.com/SurveyMonkey/skills/issues/37) closes with them.
- ~~**Rate limits at user/org scope.** Fan-out over many repos plus per-package PR checks can burn REST quota. The script batches and paginates, but very large orgs may need a `--repo-limit` guard.~~ **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): there is no fan-out to bound. A run covers the checkouts on disk, so the repo count is the user's own, and no `--repo-limit` guard is needed.
...
3. **Phase 3: org and user scope (v0.4.0, [#6](https://github.com/SurveyMonkey/skills/issues/6)).** Extend `discover-alerts.sh` with `--scope`, add push-access filtering (with skipped repos listed in the summary) and the 403 fallback. Orchestrator gains cross-repo dispatch. EMU orgs are out of scope (see Non-Goals). **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): this phase is partly reversed. `--scope`, the org and user endpoint paths, the push-access filtering, the 403 fallback and the clone phase that made a cross-repo batch actionable are all removed; a run covers the checkouts on disk, and cross-repo dispatch survives only in that form, across the repositories already cloned under the current directory.
4. **Phase 4: pin audit (v0.5.0, [#7](https://github.com/SurveyMonkey/skills/issues/7)).** Add the adapter `list_pins` verb, `check-advisories.sh`, `agents/audit-pins.md`, and the direct `commands/audit-pins.md` entry point, report-only. Wire the orchestrator's spare-slot dispatch and post-fix recommendation (both moved here from Phase 2, alongside the agent they invoke). Graduated to removal PRs in v0.8.0 ([#72](https://github.com/SurveyMonkey/skills/issues/72)), with the combined-set test the per-pin tests cannot provide; report-only stays as an explicit mode ([ADR 007](../adr/007-pin-removal-prs.md)). Scope settled during the phase: `list_pins` classifies each entry's value (`range`, `alias`, `protocol`, `reference`, `unparseable`) because only a range is a version pin; `check-advisories.sh` answers with four verdicts rather than a boolean; the audit tests one pin per install, with no batch shortcut; the audit is repo-scoped, and stays so on top of Phase 3's cross-repo dispatch — one agent audits one repository, and an org-scope batch names the repo it audits rather than auditing all of them. **Superseded in part** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): the audit is still repo-scoped, one agent to one repository, but there is no org-scope batch any more for it to sit on top of.
...
- ~~At org scope, repos skipped for lack of push access are listed by name in the summary, never silently dropped.~~ **Superseded** ([#188](https://github.com/SurveyMonkey/skills/issues/188), [ADR 011](../adr/011-scope-is-the-checkouts-on-disk.md)): there is no org scope and no push-access filtering. The never-silently-dropped requirement outlives them: every checkout excluded in phase 1 or 2 is reported by name.
