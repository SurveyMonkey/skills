...
Bands: **Low** 0-3, **Medium** 4-6, **High** 7 and above, with three escalation rules: neither a major version delta (F1 = 2) nor a newly introduced unscoped override (F6 = 2) ever rates Low, and a fix crossing two or more major lines on a runtime dependency (F2 >= 1) with no test signal (F4 = 2) never rates below High, regardless of total. A major bump of an untested, widely imported runtime dependency lands where it should (High); a patch bump of a dev-only transitive with passing tests lands at Low. These bands and thresholds ship as-is as the starting baseline; feedback from engineers reviewing scored PRs drives adjustments, recorded in the calibration ADR (see Decisions & Follow-ups). The thresholds are absolute risk points rather than proportions of the maximum, which is why adding F6 and F7 did not move them: F6 is 0 for every direct update and every scoped override, and F7 is 0 for every fix that crosses at most one major line and no pin, so a fix scoring 0 on both keeps the band it had. A fix that does score on the new factors, or trips the new escalation, changes band, and that is the point of adding them: the sweep case `bork#350` moves from Medium 6 to High 7 on exactly that route.
...
| (new) risk scoring | `score-merge-risk.sh` | Computes F1-F3 and F7 via adapter verbs, applies bands to agent-supplied F4/F5/F6 |
...
- PRs open ready for review, not as drafts. ~~**Revised during Phase 1** ([ADR 002](../adr/002-pr-draft-state-and-approval-flow.md)): PRs open as drafts and the orchestrator asks once whether to mark the batch ready. Opening ready left no checkpoint at all between approving a dispatch plan described in prose and N pull requests existing against real repositories; batch approval had removed the control point rather than relocating it. Draft state restores it for one interaction per batch. Phase 1 adopts the single-PR form, so `fix-alert.md` drops its pre-commit pause.~~ **Reverted to the original position** ([ADR 006](../adr/006-pr-ready-state-and-the-pre-dispatch-checkpoint.md), [#65](https://github.com/SurveyMonkey/skills/issues/65)): PRs open ready again. The checkpoint ADR 002 was defending is real and did not vanish with draft state; it moved ahead of `gh pr create` into the dispatch approval, which now states that approving it opens the pull requests and confirms per repo wherever auto-merge is permitted. Two things settled it: drafts notify no reviewers and no CODEOWNERS, which on a live alert makes a finished fix invisible work, and ADR 002's own field data (arsenalamerica/app#233) showed the draft checkpoint merging a PR unread wherever auto-merge was armed. The history is worth keeping in this shape: the first position was right, the objection to it was also right, and only relocating the checkpoint answers both.
...
- **The audit is per repository, at every scope.** `audit-pins` takes one repo, tests that repo's
  ...
  ([#6](https://github.com/SurveyMonkey/skills/issues/6)) that means a spare-slot audit in an
  org-scope batch names the single repo it audits (the top-ranked group's), and the post-dispatch
  recommendation offers one agent per repo the batch touched, dispatched under the same cap.
  Cross-repo dispatch changes how many audits run, never what one audit covers.
...
- ~~`mark-ready.sh` (Phase 2) has two verbs: read-only `status` and mutating `promote` (rebase, then ready only on success).~~ **Renamed and narrowed to `pr-state.sh` by [ADR 006](../adr/006-pr-ready-state-and-the-pre-dispatch-checkpoint.md)**, which retired promotion: `status` is unchanged (per-PR check state derived from `statusCheckRollup`, behind/conflict from `mergeStateStatus`, and auto-merge armed-versus-permitted), `automerge` answers the repository setting before any PR exists, and `rebase` rebases only, never marking ready and never merging, with conflicts reported and never resolved. Policy stays with the orchestrator: **a PR whose own agent scored F4 or F5 at 2 is reported as unverified**, in the summary and on the PR, because it is now reviewers rather than a promotion gate who act on that; check state is observed, never prescribed.
- **Where auto-merge is armed on a PR, that PR merges itself on green** (field data: arsenalamerica/app#233, recorded in ADR 002's consequences). ADR 006 splits the response in two: the repository setting is disclosed and confirmed **per repo before dispatch**, since a PR cannot be armed before it exists, and an armed PR is reported after creation with the command that disarms it. The orchestrator never arms auto-merge and never disarms it on the user's behalf.
...
- [ADR 002: PR draft state and the approval flow](../adr/002-pr-draft-state-and-approval-flow.md) (superseded)
- [ADR 006: PR ready state and the pre-dispatch checkpoint](../adr/006-pr-ready-state-and-the-pre-dispatch-checkpoint.md)
