...
rules in `plugins/gh-security/scripts/CLAUDE.md` and the 90 fixture directories under
...
bash and jq make the wrong thing the quiet thing. `plugins/gh-security/scripts/CLAUDE.md` records
...
- **No change to the domain rules.** Where the port disagrees with
  `plugins/gh-security/scripts/CLAUDE.md`, the document wins and the port is wrong, except for the
...
- **A type system is not a domain check.** Nothing about TypeScript catches an adapter that reads
  ...
  `plugins/gh-security/scripts/CLAUDE.md` remain the requirements document, and the fixture that
...
| P0-4 | [#216](https://github.com/SurveyMonkey/skills/issues/216) | `scripts/CLAUDE.md` rewritten; the `testing` skill's bash-specific parts amended |
...
- Requirements: `plugins/gh-security/scripts/CLAUDE.md`, `spec/fixtures/`, `spec/PINS.md`
