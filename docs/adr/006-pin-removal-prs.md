...
The branch name is fixed and **owned by this plugin**: nothing else ever writes it. That is what
makes its lifecycle mechanical rather than a judgment call. The worktree is created detached in
both modes and the branch is created only in phase 8, from inside the worktree, immediately before
the commit, so a run that opens no PR (an empty candidate set, a failed combined test, a partial
map) leaves no branch behind. Before creating it, an open PR on that head means the audit reports
and opens nothing; with no open PR, a leftover local or remote branch of that name can only be the
remnant of a merged or closed removal PR, so the local one is deleted and the remote one is
overwritten with `--force-with-lease` (which still refuses when the remote moved since the fetch,
the one case that is not a remnant). A conflicted removal PR is therefore regenerated by closing it
and re-running, with no branch for anyone to clean up by hand.
...
edits source or tests, so a check the removal breaks ends the run rather than being fixed in place. It is a
