...
**A command is tested through its exported handler**, called directly with its arguments and an
injected io and `gh` client, because that export is the seam the registry itself reaches it
through. A spawned process covers the entry point's own behavior and nothing else. The `gh`
